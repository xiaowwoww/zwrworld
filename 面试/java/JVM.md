...
**特点：** 唯一一个不会发生 OutOfMemoryError 的区域。
...
. 堆（Heap） —— 最大、最重要的内存区域
作用：存放几乎所有对象实例和数组（JVM 规范规定对象必须在堆上分配，但 JIT 优化可能逃逸分析后栈上分配）。
GC 主战场：垃圾回收器主要管理此区域。
结构（分代）：
...
. 方法区（Method Area） —— 逻辑概念，Java 8+ 由元空间实现
作用：存储类的元数据信息，包括：
类结构（字段、方法、接口）
...
Java 8 及以后：改为 元空间（Metaspace），使用本地内存（Native Memory），不再受 JVM 堆大小限制。
